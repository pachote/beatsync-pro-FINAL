...
# Global dark theme for all QMessageBox dialogs
DARK_MESSAGEBOX_STYLE = """QMessageBox { background-color: #1a1a1a; } QMessageBox QLabel { color: #ffffff; font-size: 14px; } QMessageBox QPushButton { background-color: #00d4ff; color: #1a1a1a; border: 2px solid #00d4ff; border-radius: 4px; padding: 6px 20px; font-weight: bold; } QMessageBox QPushButton:hover { background-color: #00b8e6; }"""
...
class BeatSyncUltimate(QMainWindow):
    ...
    def create_topaz_bar(self):
        ...
        self.cred_label = QLabel("Credits: Unlimited")
    ...
    def on_generation_success(self, output_path):
        ...
        # Force memory cleanup to prevent 4th video freeze
        import gc
        gc.collect()
...
Discord: discord.gg/sSukQyXA"""
    ...
    def on_video_deleted(self, thumb):
        ...
        # Reset aspect ratio when all clips deleted
        if not self.video_clips:
            self.project_aspect_ratio = None
            self.aspect_warning_label.hide()
            print('[ASPECT] Reset - all clips deleted')
...
if __name__ == "__main__":
    ...
    if sys.platform == "win32":
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("rendereelstudio.beatsyncpro.1")

    app = QApplication(sys.argv)
    app.setWindowIcon(QIcon('assets/beatsync_icon.ico'))
    ...
    if DEV_MODE:
        ...
        window.api = api
        # Update UI with real user data from API
        if api.email:
            window.account_user_btn.setText(api.email)
        if api.balance is not None:
            window.cred_label.setText(f"Credits: {api.balance}")
        window.show()
