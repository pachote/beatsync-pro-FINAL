# BeatSync PRO Ultimate - Patent Pending Trademark Pending
...
class BeatSyncUltimate(QMainWindow):
    ...
    def _update_lip_cost(self):
        quality = self.lip_combo.currentData()
        max_clips = self.lip_max_spin.value()
        if max_clips == 0:
            self.lip_cost_est.setText("Est: unlimited")
        else:
            cost_per = 1.5 if quality == "standard" else 3.0
            low = int(max_clips * cost_per * 0.7)
            high = int(max_clips * cost_per * 1.3)
            self.lip_cost_est.setText("Est: ~$" + str(low) + "-$" + str(high))
    ...
    def generate_video(self):
        ...
        try:
            ...
            self.worker = VideoGenerationWorker(selected_audio, self.video_clips,
                                               ...
                                               lip_sync_quality=self.lip_combo.currentData(),
                                               lip_sync_max_clips=self.lip_max_spin.value())
    ...
    def create_right_panel(self):
        ...
        # Max clips control
        lip_row = QHBoxLayout()
        lip_row.addWidget(QLabel("Max Clips:"))
        self.lip_max_spin = QSpinBox()
        self.lip_max_spin.setRange(0, 50)
        self.lip_max_spin.setValue(5)
        self.lip_max_spin.setFixedWidth(60)
        self.lip_max_spin.setToolTip("0 = unlimited")
        lip_row.addWidget(self.lip_max_spin)
        self.lip_cost_est = QLabel("Est: ~$5-15")
        self.lip_cost_est.setStyleSheet("color: #FFD700; margin-left: 8px;")
        lip_row.addWidget(self.lip_cost_est)
        lip_row.addStretch()
        
        # Connect signals for cost update
        self.lip_max_spin.valueChanged.connect(self._update_lip_cost)
        self.lip_combo.currentIndexChanged.connect(self._update_lip_cost)
    ...
    def on_audio_selected(self, card):
        ...
        try:
            ...
            print(f"[] Waveform loaded: {card.filename}")
            ...
            print(f"[] Beat markers loaded: {len(beat_analysis.beats)} beats, {len([b for b in beat_analysis.beats if b.is_downbeat])} downbeats")
            ...
            if cap.isOpened():
                ...
                # Reject mismatched videos
                    ...
                    QMessageBox.warning(
                        ...
                        f" Video is {aspect} but project is locked to {self.project_aspect_ratio}.\n\n" +
