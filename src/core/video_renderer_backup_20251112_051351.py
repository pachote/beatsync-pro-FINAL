...
class VideoRenderer:
    ...
    def _get_effects_filter(self, clip):
        """Apply color grading and effects to clip"""
        if not hasattr(self, 'use_effects') or not self.use_effects:
            return ""
        try:
            filters = []
