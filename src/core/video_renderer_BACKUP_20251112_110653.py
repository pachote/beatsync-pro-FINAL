...
class VideoRenderer:
    ...
    def _get_effects_filter(self, clip: Dict) -> str:
        ...
        if isinstance(motion, tuple):
            ...
            return self.camera_motion.apply_motion(motion_type, duration, intensity)
        else:
            return self.camera_motion.apply_motion(motion, duration, 0.5)
    ...
    def _render_single_clip_gpu(self, index: int, clip: Dict, temp_dir: Path) -> Path:
        ...
        if EFFECTS_AVAILABLE and self.color_engine and self.aesthetic_style != 'Natural':
            color_filter = self.color_engine.get_filter(self.aesthetic_style)
            if color_filter:
