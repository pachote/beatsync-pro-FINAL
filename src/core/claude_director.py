...
try:
    ...
    print("?? OpenAI not installed - no backup API available")
import base64
import json
...
class ClaudeAIDirector:
    def __init__(self, api_key=None):
        ...
        # FAILOVER: OpenAI backup
        self.openai_client = None
        self.openai_api_key = os.environ.get("OPENAI_API_KEY", "")
        if OPENAI_AVAILABLE and self.openai_api_key:
            try:
                self.openai_client = openai.OpenAI(api_key=self.openai_api_key)
                print("   OK OpenAI backup ready")
...
Use actual values like numbers and specific strings, not placeholders."""
        ...
        for video_path in video_files:
            try:
                ...
                # TRY CLAUDE FIRST, FAILOVER TO OPENAI
                try:
                    analysis = self._analyze_frames_with_claude(frames, analysis_system_prompt)
                except Exception as claude_error:
                    print(f"      ?? Claude API failed: {claude_error}")
                    print("      ?? Attempting OpenAI failover...")
                    analysis = self._analyze_frames_with_openai(frames, analysis_system_prompt)
                
                # Add face detection
                print(f"  [FACE DETECTION] Scanning for faces...")
                face_data = self.face_detector.analyze_video_clip(video_path, sample_frames=3)
                ...
                face_info = f" | Face: {face_data['gender']} ({face_data['confidence']:.0%})" if face_data['has_face'] else " | No face"
                print(f"✓ Energy: {prof.get('energy_level', 5)}/10 | Mood: {pers.get('mood', 'neutral')}{face_info}")
            except Exception as e:
...
5. MUSICAL SECTIONS:
   - Intro: Atmospheric, longer clips (6-8s) - BUILD ANTICIPATION
   ...
   - Outro: Wind down, hold on beautiful shots (6-10s)

🎬 CINEMATIC INTRO/OUTRO INTELLIGENCE (CRITICAL FOR PROFESSIONAL FEEL):

   **PRE-FIRST-BEAT INTRO** (time 0.0 to first major beat):
   - Start with 1-3 establishing shots BEFORE the main beat pattern starts
   - Use LOW energy clips (1-4/10) with high visual beauty
   - Longer durations: 4-8 seconds each
   - Creates anticipation and sets the mood
   - Can use slow camera movement clips, atmospheric scenes
   - DON'T just slam into the beat pattern immediately!
   
   Example intro sequence:
   [0.0-6.0s] Wide atmospheric shot (6s)
   [6.0-10.0s] Slow reveal or pan (4s)
   [10.0] → FIRST MAIN BEAT - energy kicks in!

   **POST-LAST-BEAT OUTRO** (after last major beat to song end):
   - After the final energetic section, WIND DOWN
   - Use 1-3 clips that feel like a conclusion
   - Hold on a visually striking "final shot" (8-12s)
   - Can reuse a memorable clip from earlier as a callback
   - Creates closure, doesn't just abruptly end
   
   Example outro sequence:
   [...last energetic clip ends]
   [song_duration - 15s] Transitional clip (5s)
   [song_duration - 10s] FINAL HERO SHOT - beautiful, memorable (10s) → END

   **PATTERN-SPECIFIC INTRO/OUTRO:**
   - BALANCED: Standard intro (2 clips) + outro (2 clips)
   - DYNAMIC: Quick intro (1 clip, 4s) + quick outro (1 clip, 6s)  
   - FLASH_CUTS: Minimal intro (1 clip, 2s) + abrupt outro (1 clip, 3s)
   - CHILL: Extended intro (3 clips, 15s total) + extended outro (3 clips, 18s total)
   - HYPERCUT: Almost no intro (0.5s) + no outro (cut on last beat)
...
MAKE IT BETTER! Return ONLY valid JSON."""
    ...
    def _get_basic_edit_sequence(self, clips_data, beats_data, song_analysis):
        """EMERGENCY FALLBACK: Basic beat-matched editing when ALL AI is down"""
        print("   ?? Basic mode: Random clips on beats")
        import random
        decisions = []
        clip_indices = list(range(len(clips_data)))
        ...
        while current_time < song_duration:
            clip_idx = random.choice(clip_indices)
            duration = random.uniform(2.0, 5.0)
            
            decisions.append({
                "time": current_time,
                "clip_index": clip_idx,
                "duration": min(duration, song_duration - current_time),
                "trim_start": 0.0,
                "trim_end": duration,
                "beat_type": "basic",
                "musical_section": "basic",
                "reasoning": "Basic mode - AI unavailable"
            })
    ...
    def _analyze_frames_with_openai(self, frames, system_prompt):
        """FAILOVER: Use OpenAI GPT-4 Vision when Claude is down"""
        try:
            if not self.openai_client:
                print("      ?? OpenAI client not available")
                return self._get_fallback_analysis()
            
            print("      ?? Using OpenAI GPT-4 Vision (failover)...")
            ...
            image_content.append({"type": "text", "text": "Analyze these frames. Return ONLY valid JSON."})
            ...
            print(f"      ? OpenAI failover failed: {e}")
            return self._get_fallback_analysis()
