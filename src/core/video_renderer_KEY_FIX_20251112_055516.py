...
class VideoRenderer:
    ...
    def render_music_video(self, edit_plan, music_path, output_path):
        ...
        effects_style = edit_plan.get('effects_style', 'Clean')
    ...
    def _render_single_clip_gpu(self, index: int, clip: Dict, temp_dir: Path) -> Path:
        ...
        if speed_filter:
            if effect_filter:
                effect_filter = f"{effect_filter},{speed_filter}"
            else:
                effect_filter = speed_filter
    ...
    def _get_effects_filter(self, clip):
        """Apply effects based on UI presets with camera motion and color grading"""
        try:
            filters = []
            
            # Get UI presets (fallback to defaults)
            color_preset = getattr(self, 'aesthetic_style', 'Natural')
            effects_preset = getattr(self, 'effects_style', 'Clean')
            
            # Get clip properties
            energy = getattr(clip, 'energy_level', 5)
            duration = clip.get('duration', 2.0)
            
            # ---------------------------------------
            # 1. COLOR GRADING (from UI preset)
            # ---------------------------------------
            if color_preset not in ["Natural", "Modern"]:
                color_mapping = {
                    "Cinematic": ("cinematic", 0.7),
                    "Vintage": ("vintage", 0.7),
                    "Vibrant": ("neon_dream", 0.8),
                    "Monochrome": ("high_contrast", 0.9),
                    "Neon": ("neon_dream", 1.0),
                    "Pastel": ("warm_sunset", 0.5),
                    "Cyberpunk": ("cyberpunk", 0.8),
                    "Film Noir": ("high_contrast", 0.8)
                }
                
                if color_preset in color_mapping:
                    preset_name, intensity = color_mapping[color_preset]
                    color_filter = self.color_grading.get_ffmpeg_filter(preset_name, intensity)
                    if color_filter:
                        filters.append(color_filter)
            
            # ---------------------------------------
            # 2. CAMERA MOTION (from EFFECTS preset)
            # ---------------------------------------
            motion_filter = None
            
            if effects_preset == "Clean":
                # No camera motion - pristine footage
                motion_filter = None
                
            elif effects_preset == "Subtle":
                # Very gentle Ken Burns
                motion_filter = self.camera.apply_motion(None, "ken_burns", duration, intensity=0.2)
                
            elif effects_preset == "Cinematic":
                # Classic Ken Burns with moderate intensity
                motion_filter = self.camera.apply_motion(None, "ken_burns", duration, intensity=0.4)
                
            elif effects_preset == "Music Video":
                # Zoom on high energy clips
                if energy >= 7:
                    motion_filter = self.camera.apply_motion(None, "zoom_in", duration, intensity=0.5)
                else:
                    motion_filter = self.camera.apply_motion(None, "ken_burns", duration, intensity=0.3)
                    
            elif effects_preset == "Experimental":
                # Medium shake for edgy feel
                if energy >= 6:
                    motion_filter = self.camera.apply_motion(None, "shake", duration, intensity=0.5)
                else:
                    motion_filter = self.camera.apply_motion(None, "pan_left", duration, intensity=0.4)
                    
            elif effects_preset == "Glitch":
                # Heavy shake on high energy
                if energy >= 7:
                    motion_filter = self.camera.apply_motion(None, "shake", duration, intensity=0.8)
                else:
                    motion_filter = self.camera.apply_motion(None, "shake", duration, intensity=0.5)
                    
            elif effects_preset == "Retro":
                # Slow pans for vintage feel
                import random
                pan_type = random.choice(["pan_left", "pan_right"])
                motion_filter = self.camera.apply_motion(None, pan_type, duration, intensity=0.3)
                
            elif effects_preset == "Glitch Storm":
                # Maximum chaos - intense shake
                motion_filter = self.camera.apply_motion(None, "shake", duration, intensity=1.0)
