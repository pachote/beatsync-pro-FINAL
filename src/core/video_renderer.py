...
try:
    with open(log_file, 'w', encoding='utf-8') as f:
        ...
        f.flush()
        from effects.color_grading_engine import ColorGradingEngine
        f.write("? ColorGradingEngine imported successfully\n")
        f.flush()
        
        f.write("Importing BeatReactiveEffects...\n")
        f.flush()
        from effects.beat_reactive_effects import BeatReactiveEffects
        f.write("? BeatReactiveEffects imported successfully\n")
        f.flush()
        
        f.write("Importing TransitionEffects...\n")
        f.flush()
        from effects.transition_effects import TransitionEffects
        from effects.speed_engine import SpeedEngine
        f.write('? SpeedEngine imported successfully\n')
        f.flush()
        ...
        f.flush()
        from effects.camera_motion import CameraMotion
...
# Windows: Hide console windows when running with pythonw
...
# from .speed_variations import SpeedVariationEngine, MusicSectionDetector  # DISABLED - using SpeedEngine instead
from effects.speed_engine import SpeedEngine
...
class VideoRenderer:
    def __init__(self, progress_callback=None):
        ...
        self.enabled_effects = ['Clean']  # Default
        self.effect_index = 0  # For variety
        self.speed_engine = SpeedEngine()  # FIXED: Use working SpeedEngine
    ...
    def apply_all_engines(self, clip_data, clip_index, music_duration, beat_times, previous_clip=None):
        ...
        is_drop = False  # MusicSectionDetector disabled
        is_breakdown = False  # MusicSectionDetector disabled
        ...
        # Get energy and mood for speed calculation
        energy_level = clip_data.get('energy', 5)
        mood = clip_data.get('mood', 'neutral')
        speed_multiplier = self.speed_engine.get_speed_for_clip(energy_level, mood, clip_index)
        speed_data = {
            'speed': speed_multiplier,
            'filter': self.speed_engine.get_speed_filter(speed_multiplier) if speed_multiplier != 1.0 else None,
            'description': f'{speed_multiplier}x'
        }
        # Combine all engine data
        enhanced_clip = clip_data.copy()

        # ? CRITICAL: Initialize effects dict FIRST
        if 'effects' not in enhanced_clip:
            enhanced_clip['effects'] = {}
        # CRITICAL: Preserve trim_start=0 for lip-synced clips (they're pre-trimmed!)
        if not clip_data.get('lip_synced', False):
            enhanced_clip['trim_start'] = trim_data['trim_start']
        else:
            enhanced_clip['trim_start'] = clip_data.get('trim_start', 0)
            print(f"  [LIP SYNC] Preserving trim_start=0 for lip-synced clip")
        ...
        # Check if ANY enabled effects are active (not just 'Clean')
        has_active_effects = any(e != 'Clean' for e in self.enabled_effects)
        if EFFECTS_AVAILABLE and self.beat_effects and has_active_effects:
            # Convert beat_strength (0-1) to energy_level (1-10)
            energy_level = int(beat_strength * 9) + 1
            # Select effect from enabled effects pool (with variety)
            if hasattr(self, 'enabled_effects') and self.enabled_effects:
                # Use different effect for each clip (variety!)
                effect_type = self.enabled_effects[clip_index % len(self.enabled_effects)]
            else:
                effect_type = 'beat_flash'  # Fallback
            
            # Calculate clip timeline position
            clip_start = enhanced_clip.get('start_time', 0)
            clip_duration = enhanced_clip.get('duration', 2.0)
            clip_end = clip_start + clip_duration
            
            # Get beat times from audio analysis
            beat_times = self.audio_data.get('beats', [])
            
            print(f"   ?? Applying {effect_type} | Clip: {clip_start:.2f}-{clip_end:.2f}s | Beats: {len(beat_times)}")
            
            # FIXED: Pass timing parameters
            beat_effect = self.beat_effects.get_energy_based_effect(
                energy_level, 
                effect_type,
            ...
            if beat_effect is not None and beat_effect != "":

                # Ensure effects dict exists (safety check)
                if 'effects' not in enhanced_clip:
                    enhanced_clip['effects'] = {}
                enhanced_clip['effects']['beat_reactive'] = beat_effect
                print(f"   ?? Beat effect applied: {type(beat_effect).__name__} = {str(beat_effect)[:80]}")
            else:
                print(f"   ?? Beat effect FAILED - got: {repr(beat_effect)} (type: {type(beat_effect).__name__})")
        ...
        if EFFECTS_AVAILABLE and self.color_engine and self.aesthetic_style != 'Natural':
            ...
            color_filter = self.color_engine.get_ffmpeg_filter(self.aesthetic_style.lower(), intensity=1.0)
            
            if color_filter is not None and color_filter != "":

                enhanced_clip['effects']['color_grading'] = color_filter
                print(f"   ?? Color grading applied: {self.aesthetic_style} = {str(color_filter)[:80]}")
            else:
                print(f"   ?? Color grading FAILED - got: {repr(color_filter)}")
    ...
    def render_music_video(self, edit_plan, music_path, output_path):
        ...
        aesthetic_style = 'Natural'  # Always Natural - no color grading  # UI uses 'color' key
        effects_input = edit_plan.get('effects', 'Clean')  # UI uses 'effects' key
        
        # Handle both list (new multi-select) and string (old single-select)
        if isinstance(effects_input, list):
            self.enabled_effects = effects_input  # Store full list
            effects_style = effects_input[0] if effects_input else 'Clean'  # Use first as default
            print(f"   ?? Multi-effects enabled: {len(effects_input)} effects selected")
            print(f"      Effects pool: {effects_input}")
        else:
            self.enabled_effects = [effects_input]  # Convert to list
            effects_style = effects_input
            print(f"   ?? Single effect mode: {effects_style}")
    ...
    def render_video(self, clips: List[Dict], music_path: str, output_path: str, aesthetic_style: str = "Modern", effects_style: str = "Clean"):
        ...
        for i, clip in enumerate(clips):
            ...
            if not os.path.exists(video_path):
                ...
                continue
            print(f"    [DEBUG] Validating: {os.path.basename(video_path)}")
    ...
    def _render_single_clip_gpu(self, index: int, clip: Dict, temp_dir: Path) -> Path:
        ...
        video_path = clip['video_path']
        print(f'  [CLIP {index}] SOURCE: {video_path}')
...
# FIX #3: Try NVENC first, fallback to CPU if it fails
    ...
    def _render_clip_nvenc(self, video_path, trim_start, duration, effect_filter, output_path):
        ...
        try:
            result = subprocess.run(cmd, check=True, capture_output=True, timeout=300, creationflags=SUBPROCESS_FLAGS)
        except subprocess.TimeoutExpired:
            print(f'  [SKIP] Clip timed out, skipping: {video_path}')
            return None
    ...
    def _render_clip_cpu(self, video_path, trim_start, duration, effect_filter, output_path):
        ...
        subprocess.run(cmd, check=True, capture_output=True, timeout=3600, creationflags=SUBPROCESS_FLAGS)
    ...
    def _concatenate_clips(self, clip_paths: List[Path], output_path: Path):
        ...
        # Filter out None values from failed/timed out clips
        clip_paths = [p for p in clip_paths if p is not None]
        print(f"  Valid clips after filtering: {len(clip_paths)}")
        ...
        result = subprocess.run(cmd, check=True, capture_output=True, timeout=3600, creationflags=SUBPROCESS_FLAGS)
